 ...
 *
 * @param metadata Указатель на метаданные прошивки
 * @param update_params Указатель на параметры обновления
...
uint32_t FirmwareUpdate_EndUpdate() {
    ...
    // Вычислить CRC всей прошивки из SPI Flash
...
void DumpFirmwareHex(flash* spiFlash, uint32_t startAddress, uint32_t length, uint8_t bytesPerLine, uint32_t delayMs)
{
    ...
    STM_LOG("Firmware dump from SPI Flash:");
    STM_LOG("Start address: 0x%08lX, Length: %lu bytes", startAddress, length);
    STM_LOG("--------------------------------------------------");
    ...
    for (uint32_t offset = 0; offset < length; offset += bytesPerLine)
    {
        ...
        {
            STM_LOG("--------------------------------------------------");
            STM_LOG("Address     : 00 01 02 03 04 05 06 07  08 09 0A 0B 0C 0D 0E 0F | ASCII");
            STM_LOG("--------------------------------------------------");
        }
    }

    STM_LOG("--------------------------------------------------");
    STM_LOG("Dump completed");
}

/**
 * @brief Выводит метаданные прошивок из Main и Backup областей
 */
void FirmwareUpdate_PrintMetadata(void)
{
    ...
    for (size_t i = 0; i < 2; i++)
    {
        ...
        g_spiFlash->W25qxx_ReadBytes(addresses[i] + METADATA_OFFSET, buffer, sizeof(meta_t));
        memcpy(&metadata, buffer, sizeof(meta_t));

        if (metadata.key_start == METADATA_KEY && metadata.version != 0 && metadata.version != 0xFFFFFFFF)
        {
            STM_LOG("%s at 0x%08lX: Version 0x%08lX, Name: %s", 
                    names[i], addresses[i], metadata.version, metadata.name_proj);
        }
        else
        {
            STM_LOG("%s at 0x%08lX: No valid metadata", names[i], addresses[i]);
        }
    }
}
