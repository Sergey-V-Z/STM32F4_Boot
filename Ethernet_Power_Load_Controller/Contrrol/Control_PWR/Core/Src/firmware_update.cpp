...
uint32_t FirmwareUpdate_EndUpdate() {
    ...
    {
        // заполнить структуру хранения нужными данными
        secondaryConfig.cells[g_updateContext.update_params.cell_num].fw_size = g_updateContext.update_params.fw_size;
        secondaryConfig.cells[g_updateContext.update_params.cell_num].fw_crc = g_updateContext.update_params.fw_crc;
        //secondaryConfig.cells[g_updateContext.update_params.cell_num].load_permission = 1; // пока что не запускаем

        // сохранить ее
        if (SetSecondaryFirmwareConfig(&secondaryConfig) != 0) {
            STM_LOG("Failed to save secondary firmware config");
            g_updateContext.status = UPDATE_STATUS_ERROR;
            g_updateContext.error = UPDATE_ERROR_FLASH_FAILURE;
            xSemaphoreGive(g_updateContext.mutex);
            return UPDATE_ERROR_FLASH_FAILURE;
