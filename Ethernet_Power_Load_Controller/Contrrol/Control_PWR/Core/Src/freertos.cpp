...
uint16_t TX_buff[UART_TX_LENGTH]={0}; // buff
uint16_t RX_buff[UART_RX_LENGTH]={0}; // buff
...
void mainTask(void const * argument)
{
  ...
  //нициализируем логгер
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&huart1, (uint8_t*)RX_buff, UART_RX_LENGTH);
        ...
        for (int var = 0; var < MAX_ADR_DEV; ++var) {
            ...
            if((devices[var].Addr >= START_ADR_I2C) && (devices[var].Addr <= (START_ADR_I2C + MAX_ADR_DEV))){
                ...
                if (evt.status == osEventMessage){
                    ...
                    parse_uart_packet(message_rx, size, &cmd, rx_data, &rx_data_size); // парсим пакет
                    if (cmd != cmd_t::data)
                    {
                        continue;
                    }
...
{
    cmd_t cmd_received = cmd_t::data;
    uint16_t data_len = 0;
    uint8_t rx_data[10];

    // Перебираем все возможные адреса устройств
    uint16_t found_count = 0;
    for (uint8_t i = 0; i < size_dev; ++i) {
        uint8_t addr = START_ADR_I2C + i;

        // Формируем пакет для запроса статуса устройства
        uint8_t tx_buf[8] = {0};
        tx_buf[0] = 0;
        tx_buf[1] = 0; 
        tx_buf[2] = 0; 
        ...
        // Отправляем команду r_status
        HAL_StatusTypeDef status = send_uart_packet(addr, cmd_t::r_status, tx_buf, sizeof(tx_buf));
        if (status != HAL_OK) {
            continue; // Если ошибка передачи, пропускаем адрес
        }

        // Ожидаем ответ через очередь сообщений (например, 5 секунд)
        osEvent evt = osMessageGet(rxDataUART1Handle, 100);
        if (evt.status != osEventMessage) {
            continue; // Нет ответа - пропускаем адрес
        }

        // evt.value.v содержит размер принятого сообщения
        uint16_t rx_size = evt.value.v; // размер принятого пакета

        if(!parse_uart_packet(message_rx, rx_size, &cmd_received, rx_data, &data_len))
        {
            continue;
        }

        if (cmd_received != cmd_t::r_status) {
            STM_LOG(LOG_ERR "Unexpected command %d for address %d", cmd_received, addr);
            continue; // Неверная команда - пропускаем адрес
        }

        // Проверяем размер данных
        if (rx_size < data_len + 4) {
            STM_LOG(LOG_ERR "Invalid data size %d for address %d", rx_size, addr);
            continue; // Неверный размер данных - пропускаем адрес
        }

        // Заполняем структуру dev[i] данными из принятого пакета
        dev[i].Addr = addr; // Устанавливаем адрес устройства
        dev[i].AddrFromDev = rx_data[0]; // Адрес устройства из ответа
        dev[i].TypePCB = (PCBType)rx_data[1]; // Тип платы
        //dev[i].StatFlash = rx_data[4]; // Статус флеш-памяти
        if(rx_data[3] == 3){
...
// функция для отправки пакетв по uart
HAL_StatusTypeDef send_uart_packet(uint8_t addr, cmd_t cmd, uint8_t *data, uint16_t data_len)
{
    if (data == nullptr || data_len == 0) {
        STM_LOG(LOG_ERR "Data buffer is null or length is zero");
        return HAL_ERROR;
    }

    // Формируем пакет: [cmd][size][data...][crc_lo][crc_hi]
    if (data_len > UART_TX_LENGTH - 4) {
        STM_LOG(LOG_ERR "Data length too large");
        return HAL_ERROR;
    }

    TX_buff[0] = static_cast<uint8_t>(cmd);
    TX_buff[1] = static_cast<uint8_t>(data_len);
    memcpy(&TX_buff[2], data, data_len);

    uint16_t crc = crc16_fast((uint8_t*)TX_buff, (2 + data_len));
    TX_buff[2 + data_len] = crc & 0xFF;
    TX_buff[3 + data_len] = (crc >> 8) & 0xFF;

    uint16_t address = addr | 0x0100;
    HAL_StatusTypeDef status;

    status = HAL_UART_Transmit(&huart1, (uint8_t*)&address, 1, 100);
    if (status != HAL_OK) return status;

    status = HAL_UART_Transmit(&huart1, (uint8_t*)TX_buff, 4 + data_len, 100);
    return status;
}
...
{
    if (data == nullptr || cmd == nullptr || data_out == nullptr || data_out_len == nullptr || data_len < 4) {
        return false; // Минимальный размер пакета: 2 байта команды + 1 байт размера + 2 байта CRC
    }

    //пакет: [cmd][size][data...][crc_lo][crc_hi]

    // Проверка контрольной суммы
    uint16_t crc_received = (data[data_len - 2] | (data[data_len - 1] << 8));
    uint16_t crc_calculated = crc16_fast(data, data_len - 2);

    if (crc_received != crc_calculated) {
        return false; // Контрольная сумма не совпадает
    }

    *cmd = static_cast<cmd_t>(data[0]); // Первая часть - команда
    uint8_t size = data[1];              // Вторая часть - размер данных

    if (size > data_len - 4) {
        return false; // Размер данных превышает доступный размер
    }

    *data_out_len = size;
    
    data_out = &data[2]; // Данные начинаются с 2-го байта

    return true; // Пакет корректен
}
...
// функция проверки пришедших данных, преобразование их из буфера в структуру ret_pwm_ch_t затем сверки этой структуры с данными из DEV_t (сравнивать только те данные которые есть в ret_pwm_ch_t). принимает указатель на буфер, указатель на DEV_t. возвращает uint8_t true если данные корректны и совпадают, иначе false
uint8_t verify_ret_pwm_data(const uint8_t *buff, const DEV_t *dev) {
    if (buff == nullptr || dev == nullptr) {
        return false;
    }

    ret_pwm_ch_t received;
    clear_ret_pwm_data(&received);

    // Десериализуем данные из буфера
    deserialize_ret_pwm_data((uint8_t*)buff, &received);

    // Сравниваем только те поля, которые есть в ret_pwm_ch_t и DEV_t
    // Проверяем enable флаги
    if (received.en1 != dev->ch[0].On_off) return false;
    if (received.en2 != dev->ch[1].On_off) return false;
    if (received.en3 != dev->ch[2].On_off) return false;
    if (received.en4 != dev->ch[3].On_off) return false;
    if (received.en5 != dev->ch[4].On_off) return false;
    if (received.en6 != dev->ch[5].On_off) return false;

    // Проверяем PWM значения
    if (received.PWM1 != dev->ch[0].PWM_out) return false;
    if (received.PWM2 != dev->ch[1].PWM_out) return false;
    if (received.PWM3 != dev->ch[2].PWM_out) return false;
    if (received.PWM4 != dev->ch[3].PWM_out) return false;
    if (received.PWM5 != dev->ch[4].PWM_out) return false;
    if (received.PWM6 != dev->ch[5].PWM_out) return false;
...
HAL_StatusTypeDef send_pwm_ch_to_dev(const DEV_t *dev)
{
    ...
    return send_uart_packet(dev->Addr, cmd_t::data, buff, size);
}
